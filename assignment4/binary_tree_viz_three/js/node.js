...
// Node in the tree
function Node(val, person, group, x, y) {
  this.value = val;
  this.left = null;
  this.right = null;
  this.person = person;
...
Node.prototype.visit = function(parent) {
  ...
  if (this.person == 1) colorS = 0xFD3057; // lisa
  if (this.person == 0) colorS = 0x31BB68; // pierre
...
Node.prototype.addNode = function(n) {
  ...
  if (n.value < this.value) {
    ...
    if (this.left == null) {
      ...
      this.left.x = this.x - (CELLWIDTH / Math.pow(2, n.distance));
      this.left.y = this.y + (CELLHEIGHT / 12);
    // Keep going!
    } else {
      n.distance++;
      this.left.addNode(n)
    ...
    if (this.right == null) {
      this.right = n;
      this.right.x = this.x + (CELLWIDTH / Math.pow(2, n.distance));
      this.right.y = this.y + (CELLHEIGHT / 12);
    // Keep going!
    } else {
      n.distance++;
